...
const DisplayPrevGames = (props)=>{
  ...
  useEffect(() => {
      ...
      let displayGames = []
      games.map(game => {
        if ((game.season.id === seasonId && game.teamNames.homeTeamId === teamId && game.halfTime === 5) || ((game.season.season === 'all' || game.season.season === '') && game.teamNames.homeTeamId === teamId && game.halfTime === 5) ) {
