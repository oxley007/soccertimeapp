...
const InputTeamName = (props)=>{
  ...
  const checkInput = () => {
    ...
    */
      ...
      return (
        <Box mt="3">
          <Button size="md" minW="100%" variant="subtle" bg="tertiary.300" onPress={() => addTeamButton()}>Add Team</Button>
        </Box>
      )
  }
    ...
    */
    ...
    inputs.map(input => {
      if (input.value === '') {
        ...
        teamNamesArray.unshift({id: teamNamesLength, teamId: teamId, teamName: input.value, teamNameShort: teamShortName, teamType: teamType, teamSelected: false, grade: ageGrade});
        ...
        userRef.doc("teamNames").update({
          ...
          firestore().collection(teamId).doc(teamId).set({
             id: teamNamesLength, teamId: teamId, teamName: input.value, teamNameShort: teamShortName, teamType: teamType, teamSelected: false
           })
