...
const doubleright = <IconAnt name="doubleright" size={20} color="#333" />;
...
const SelectPlayersDragDrop = (props) => {
  ...
  const dispatch = useDispatch()
	...
	useEffect(() => {
		...
		console.log(props.addPlayerPage  + ' props.addPlayerPage hit here into this ');

		console.log(checkSort  + ' checkSort check ');

		console.log(JSON.stringify(props.posArray) + ' props.posArray need to review on each click.');
		console.log(JSON.stringify(knightPosArray) + ' knightPosArray need to review on each click.');
	...
	useEffect(() => {
			...
			console.log('when is this hit?');

			const checkSort = checkSort + 1
			//setCheckSort(checkSort)
			console.log(checkSort + ' check checkSort');
...
useEffect(() => {
	...
	const getPosArrayStats = () => {
		...
		console.log(JSON.stringify(posArray) + ' posArray need more.')
	...
	if (player.row === 0 && player.column > 1) {
		...
		return (
			<Box bg="fuchsia.200" minW="90%" minH="50%" shadow="9" rounded="md">
					<Text style={{textAlign: 'center', justifyContent: 'center', alignItems: 'center', paddingTop: '4%'}}>{player.initials[0]}{player.initials[1]}</Text>
			</Box>
		)
	}
			...
			return (
					<Box style={{textAlign: 'flex-start'}} pb="3">
						{isFirstGame === true &&
							...
							<Text style={{fontSize: 11, color: '#fff', fontWeight: '700'}}>IMPORTANT! Drag players into position. Also, drag any players who are absent or not playing into the <Text style={{color: '#fb7185'}}>RED</Text> area at the bottom of the feild.</Text>
					</Box>
			)
		}
		else {
						...
						color: "white"
							...
							<Center>
								<LinearGradient start={{x: 0, y: 0}} end={{x: 1, y: 0}} colors={['#d1fae5', '#34d399']} style={styles.linearGradientEventBoard}>
									<Text >{playerPositionDisplayBoardText} {playerPositionDisplayBoardLatest}</Text>
								</LinearGradient>
							</Center>
						</Center>
						</Box>
    ...
    games[0].teamPlayers.map(player => {
      ...
      if (player.currentPosition !== 'sub') {
       playerOnFeild++
		...
		console.log(JSON.stringify(knightPosArray) + ' what is knightPosArray 2')

		console.log(playerOnFeild + ' playerOnFeild == what?');

		if (playerOnFeild === 0) {
			Alert.alert("Please drag at least one player into position on the playing feild before you can compare players.")
		}
		else {
			const checkSortNew = checkSort + 2
		...
		<DraxProvider>
			<View style={styles.container}>
				...
				<ImageBackground source={require(`../../assets/soccerFeild-drag-drop-five-with-red.png`)} resizeMode="stretch" imageStyle={{ }} style={styles.backgroundImage}>
					...
					{props.sortPage !== true &&
						<View>
							<Button minW="100%" variant="unstyled" size="md" mt="3" mb="3" pt="5" pb="5" _text={{fontSize: 26, color: '#fff'}} variant="subtle" onPress={() => compareButton()}>
								<HStack>
									<Center>
										<Text style={styles.textEighteen}>Compare Player Time/Stats {doubleright}</Text>
									</Center>
								</HStack>
							</Button>
