...
useEffect(() => {
 ...
 console.log(JSON.stringify(props.playerData) + ' props.playerData.');
  ...
  const getPositionTimesDisplay = () => {
   ...
   console.log(stats.season + ' stats.season check');
    const currentSeasonCheck = stats.season

    console.log('do i get here?');

    console.log(JSON.stringify(stats[0]) + ' .posTimes real just check stat [1]');
   ...
   //console.log(currentSeasonCheck + ' stat[0].season is? (works from coash player stats)');
   console.log(currentSeasonCheck + ' currentSeasonCheck is?');
   console.log(props.seasonId + ' props.seasonId is?');


      if (currentSeasonCheck === props.seasonId || (props.whereFromPlayer === 191 || props.whereFromPlayer === 181)) {
     console.log(' should be getting here? ');
        ...
        let subData = []
    try {
   console.log(JSON.stringify(stats.posTimes.fwd) + ' [0] playerData firsy fws check.. (works from coash player stats)');
   console.log(props.whereFromPlayer + ' props.whereFromPlayer is???');
   //console.log(JSON.stringify(fwd) + ' stat.fwd check.)');
      ...
      if (props.whereFromPlayer === 191) {
        ...
        console.log('should be hittin here fro my new setup..');
          /*
          ...
          */
          ...
          fwdData = stats.posTimes.fwd
          midData = stats.posTimes.mid
          defData = stats.posTimes.def
          golData = stats.posTimes.gol
          subData = stats.posTimes.sub
      }

      fwdData.map(fwd => {
      console.log(JSON.stringify(stats.posTimes.fwd) + ' playerData firsy fws check.. 1');
        const totalTime = getPositionTimes(fwd)
      console.log(JSON.stringify(stats.posTimes.fwd) + ' playerData firsy fws check.. 2');
 ...
 console.log('do i even get to here?');
   ...
   console.log('do i even get to here? 2');
     ...
     console.log(JSON.stringify(stats.posTimes.mid) + ' firsy mid check new new.');
          fwdData = stats.posTimes.fwd
          midData = stats.posTimes.mid
          defData = stats.posTimes.def
          golData = stats.posTimes.gol
    ...
    console.log(JSON.stringify(stats.posTimes.mid) + ' firsy mid check new.');
      midData.map(mid => {
     console.log(mid + ' mid check.');
        const totalTime = getPositionTimes(mid)
        console.log(totalTime + ' totalTime mid check.');
        const totalTimeNum = Number(totalTime)
        console.log(totalTime + ' totalTimeNum mid check.');
    ...
    console.log(JSON.stringify(stats.posTimes.def) + ' firsy def check new.');
      defData.map(def => {
    ...
    console.log(JSON.stringify(stats.posTimes.gol) + ' firsy gol check new.');
      golData.map(gol => {
    ...
    console.log(JSON.stringify(stats.posTimes.sub) + ' firsy sub check new.');
      subData.map(sub => {
 ...
 console.log(JSON.stringify(midTimeArray) + ' midTimeArray just got check now inside.');
  ...
  */
  console.log('i need this to look at.. ' + JSON.stringify(statsSort));
      ...
      return (
        ...
        return (
          ...
          {props.whereFromPlayer === 191 &&
            ...
            {games.length >= 1 && statePlayerIndex === true && typeToDisplay !== 2 &&
            <Box maxW="100%">
            ...
            <Text style={{color: '#fff', fontSize: 16}}>{JSON.stringify(statsSort)}</Text>
