      ...
      const checkSubscription = async (productIdentification) => {
       ...
       //console.log(JSON.stringify(customerInfo.activeSubscriptions) + ' check activeSubscriptions');
         ...
         //console.log('hit if subscriptrion avail.');
            ...
            if (productIdentification === 'pro_yearly_indiv' || productIdentification === 'pro_yearly_indiv_android:yearly-sub-799') {
           //console.log('am i hitting into here, ya?');
              pro_yearly_indiv[0].purchased = false
              pro_yearly_indiv[0].expiryDate = null
            }
            else if (productIdentification === 'pro_yearly_team' || productIdentification === 'pro_yearly_team_android:yearly-sub-69') {
                pro_yearly_team[0].purchased = false
                pro_yearly_team[0].expiryDate = null
            }
            else if (productIdentification === 'pro_yearly_player' || productIdentification === 'pro_yearly_player:yearly-sub-7-99') {
 ...
 */
   ...
   //console.log(JSON.stringify(storeProductId) + ' output prod.');
     ...
     //console.log('hit my catch teamanems pro_forever_player');
      ...
      else if (storeProductId === 'pro_forever_indiv' ) {
     ...
     //console.log('hit my catch teamanems pro_forever_player');
      ...
      else if (storeProductId === 'pro_forever_team') {
     ...
     //console.log('hit my catch teamanems pro_forever_player');
      ...
      else if (storeProductId === 'pro_forever_player') {
    ...
    const startNewGameNew = async (fromWhere) => {
 ...
 //console.log(gameDate + ' gameDate check');
    ...
    const gameStatusNew = () => {
      ...
      let gameStatusDesc = ''
          ...
          if (getHideCircle === true) {
            ...
            if (userProfile === 4) {
              gameStatusText = 'Awiting Subs Setup by Coach...'
              ...
              gameStatusDesc = 'You can now begin setting up subs management'
            }
          }
          else if (tempGameComplete === false && games[0].gameSetup === false) {
            if (games[0].gameSetupProfile === currentUser.uid) {
              gameStatusText = 'Continue Subs Management'
              gameStatusDesc = 'Continue with your Subs Management to view and manage substitutions.'
            }
            else {
  ...
  if (getHalfTimeFlag === 5 || tempGameComplete === true) {
    if (userProfile === 4) {
     ...
     if (userProfile === 1) {
      ...
      else*/ if (numberOption === 4)
      {
        return (
          <Button bg="transparent" p="0" onPress={() => proPage()}>
            <Box bg="#222" mb="0">
              <Box style={{backgroundColor: '#FACC15', maxWidth: '100%', minWidth: '100%', borderTopLeftRadius: 5, borderTopRightRadius: 5, paddingTop: 5, justifyContent: 'center', alignItems: 'center', paddingBottom: 5, paddingLeft: 15, paddingRight: 15}}><Text style={{fontSize: 16, fontWeight: '600'}}>Step {numberOption} (Optional)</Text></Box>
            </Box>
          </Button>
        )
     }
...
/*
   ...
   */
   ...
   const sendCoachId = () => {
    ...
    {userProfile === 1 &&
    <Box alignItems="center" mt="3" shadow="6">
      <ImageBackground source={require(`../../assets/soccerballpattern-leftcrop-trans.png`)} style={styles.backgroundImageCoach}>
      {proBanner(2)}
        <LinearGradient start={{x: 0, y: 0}} end={{x: 1, y: 0}} colors={['rgba(0,0,0,0.3)', 'rgba(0,0,0,0.3)']} style={styles.linearGradient}>
          <Button minW="100%" size="md" variant="subtle" _text={{
                  ...
                  fontSize: 25,
                  fontWeight: '500'
                }} style={{justifyContent: 'flex-start'}} bg="transparent" pt="3" pb="3" onPress={() => startNewGameNew(1)}>
              <HStack maxW="100%" style={{textAlign: 'left', alignItems: 'left', justifyContent: 'left', width: '100%'}}>
                <HStack minW="10%">
                  <VStack  pt="2">
                  {getHideCircle === true &&
            ...
            <LinearGradient start={{x: 0, y: 0}} end={{x: 1, y: 0}} colors={['rgba(0,0,0,0.3)', 'rgba(0,0,0,0.8)']} style={styles.linearGradient}>
              ...
              <HStack minW="100%" maxW="100%">
                ...
                <HStack minW="78%" maxW="78%">
                  <VStack pl="5">
                    <HStack p="1" pl='0'>
                      <Text style={{fontSize: 16, color: '#fff', fontWeight: '700', lineHeight: 16}}>Send Invites to Parents</Text>
                    </HStack>
                    <HStack p="0.5" pl='0'>
                      <Text style={{fontSize: 14, color: '#ccc', lineHeight: 14}}>Send invites to parents so they can view live scores!</Text>
                    </HStack>
                  </VStack>
                </HStack>
                <HStack minW="10%">
